...
public record BlobDescriptor(
    ...
    public BlobDescriptor(
        ...
        this(path, mimeType, createdBy, createdOn, size, compression, Map.copyOf(attributes), Can.empty());
    }

	public BlobDescriptor withAttribute(final String key, final String value) {
		var map = new HashMap<>(attributes());
		map.put(key, value);
		return this.withAttributes(map);
	}
