 ...
 *  under the License.
 */
package io.github.causewaystuff.commons.base.listing;
...
public record Listing<T>(
    ...
    public <K> Listing<T> merge(@NonNull final MergePolicy policy, @Nullable final Listing<T> newerVersion) {
        ...
        .forEach(line->{
            ...
            switch (line) {
                case LineComment comment -> mergedLines.add(line); // identity operation
                case LineEnabled<?> lineEnabled -> {
                ...
                case LineDisabled<?> lineDisabled -> {
                    if(incomingContainsKey) {
        ...
        if(!incomingByKey.isEmpty()) {
            mergedLines.add(new LineComment("#MERGED"));
